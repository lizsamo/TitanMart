...
struct CheckoutView: View {
    ...
    // New state variables for enhanced meetup system
        ...
        NavigationView {
            ScrollView {
                VStack(spacing: Spacing.lg) {
                    ...
                    // ENHANCED: Meeting Location Card
                    VStack(alignment: .leading, spacing: Spacing.md) {
                        HStack {
                        ...
                        if meetingLocation.isEmpty {
                            ...
                            .padding(Spacing.md)
                            .background(Color.titanBlue.opacity(0.05))
                            .cornerRadius(CornerRadius.small)
                        }

                        // Schedule Button (only show if location is selected)
                        if !meetingLocation.isEmpty {
                            Button(action: {
                                ...
                                HStack {
                                    ...
                                    if let meetingTime = meetingTime {
                                        Text("Scheduled: \(meetingTime, style: .date) at \(meetingTime, style: .time)")
                                    } else {
                                        Text("Schedule Meetup Time")
                                    }
                                    Spacer()
                        ...
                        HStack(spacing: Spacing.xs) {
                            ...
                            Text("Choose a campus spot and schedule a meetup time")
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                    }
            ...
            .alert("Payment Successful!", isPresented: $showingSuccess) {
                ...
                if let meetingTime = meetingTime {
                    Text("Your payment has been processed! Meet the seller at \(meetingLocation) on \(meetingTime, style: .date) at \(meetingTime, style: .time).")
                } else {
            ...
            .sheet(isPresented: $showingLocationPicker) {
                LocationPickerView(selectedLocation: $meetingLocation)
            }
            // Meetup Scheduler Sheet
    ...
    private func placeOrder() {
        ...
        Task {
            do {
                // Create order with meetup information
                var orderItems = cartService.items
                
                // Create the order first (to get orderId)
                let order = try await orderViewModel.createOrder(items: orderItems)
                
                // In a real app, we would update the order with meetup details here
                if !meetingLocation.isEmpty {
                    print("📍 Meetup scheduled: \(meetingLocation) at \(meetingTime?.description ?? "TBD")")
                }
